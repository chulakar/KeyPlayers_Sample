...
def cxSelfPoints(ind1, ind2):
    #print " ".join(map(str, ["ind1 - ", ind1, "\n Length - ", str(len(ind1))]))
    #print " ".join(map(str, ["ind2 - ", ind2, "\n Length - ", str(len(ind2))]))
    length = len(ind1)
    ind1_copy = copy.deepcopy(ind1)
    
    random.seed(time.clock())
    p_ind1 = random.randint(5, length-5)
    #print p_ind1
    random.seed(time.clock())
    p_1_ind1 = random.randint(3, p_ind1)
    #print p_1_ind1
    random.seed(time.clock())
    p_2_ind1 = random.randint(p_ind1+1, length-3)
    #print p_2_ind1
    
    ind1[p_1_ind1:p_1_ind1+(len(ind1)-p_2_ind1)] = ind1_copy[p_2_ind1:]
    ind1[p_1_ind1+(len(ind1)-p_2_ind1):p_1_ind1+(len(ind1)-p_2_ind1)+(p_2_ind1-p_ind1)] = ind1_copy[p_ind1:p_2_ind1]
    ind1[p_1_ind1+(len(ind1)-p_2_ind1)+(p_2_ind1-p_ind1):] = ind1_copy[p_1_ind1:p_ind1]
...
def cxOnePoint(ind1, ind2):
    ...
    for val in ind1:
        if (val==1):
            ind1points.append(index)
            temp = temp+val
        else:
            ind0points.append(index)
        index = index+1
    if temp > 5:
        excess = temp-5
        random.seed(time.clock())
        ini_nodes = random.sample(set(ind1points), excess)
        for valindex in ini_nodes:
            ind1[valindex] = type(ind1[valindex])(not ind1[valindex])


    if temp < 5:
        remainder = 5 - temp
        random.seed(time.clock())
        ini_nodes_o = random.sample(set(ind0points), remainder)
        for valindex_o in ini_nodes_o:
            ind1[valindex_o] = type(ind1[valindex_o])(not ind1[valindex_o])
    ...
    if temp > 5:
        ...
        ini_nodes = random.sample(set(ind1points), excess)
        for valindex in ini_nodes:
            ind2[valindex] = type(ind2[valindex])(not ind2[valindex])
    ...
    if temp < 5:
        ...
        ini_nodes_o = random.sample(set(ind0points), remainder)
        for valindex_o in ini_nodes_o:
            ind2[valindex_o] = type(ind2[valindex_o])(not ind2[valindex_o])
    #print "ind1 - "+str(ind1)
    #print "ind2 - "+str(ind2)
    return ind1, ind2
...
def mutFlipBit(individual, indpb):
    ...
    if temp > 5:
        ...
        ini_nodes = random.sample(set(ind1points), excess)
        for valindex in ini_nodes:
            individual[valindex] = type(individual[valindex])(not individual[valindex])
    ...
    if temp < 5:
        ...
        ini_nodes_o = random.sample(set(ind0points), remainder)
        for valindex_o in ini_nodes_o:
            individual[valindex_o] = type(individual[valindex_o])(not individual[valindex_o])
